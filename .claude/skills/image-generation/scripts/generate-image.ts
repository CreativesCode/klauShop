...
if (!prompt) {
  ...
  console.error(
    "  --aspect    Aspect ratio: 1:1, 16:9, 9:16, 4:3 (default: 1:1)",
  );
...
function loadEnvKey(): string {
  ...
  console.error(
    "ERROR: OPENROUTER_API_KEY not found in .env.local, .env, or environment",
  );
  ...
  const response = await fetch(
    "https://openrouter.ai/api/v1/chat/completions",
    {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://titan-factory.dev",
        "X-Title": "Titan Factory Image Generation",
      },
      body: JSON.stringify({
        model,
